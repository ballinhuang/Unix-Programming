...
void printIPV4Connections(string ipv4_path)
{
    /*
    ...
    */
    ...
    if (file)
    {
        ...
        while (fscanf(file, "%*s%x:%x%x:%x%*s%*s%*s%*s%*s%*s%s%*[^\n]\n", &src_addr.s_addr, &src_port, &dis_addr.s_addr, &dis_port, inode) != -1)
        {
            ...
            cout << left << setw(6) << "tcp"
                 << setw(25) << string(src_ip) + ":" + porthex2string(src_port)
                 << setw(25) << string(dis_ip) + ":" + porthex2string(dis_port);
...
void printIPV6Connections(string ipv6_path)
{
    /*
...
void printConnections(string type, string filter)
{
    ...
    printIPV4Connections(ipv4_path);
    printIPV6Connections(ipv6_path);
}
