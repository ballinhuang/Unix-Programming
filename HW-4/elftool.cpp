...
elf_open(const char *filename)
{
    ...
    if ((elf = elf_begin(fd, ELF_C_READ, NULL)) == NULL)
        goto err_quit;
    if ((ident = elf_getident(elf, &idsize)) == NULL)
        goto err_quit;
    if ((e = (elf_handle_t *)malloc(sizeof(elf_handle_t))) == NULL)
        goto err_quit;
...
{
    if (elf_load_phdr(e) == NULL)
        goto err_quit;
    if (elf_load_shdr(e) == NULL)
        goto err_quit;
    if (elf_load_strtab(e) == NULL)
        goto err_quit;
...
#define ASSIGN_SYM(x)                                                                                                   \
    do                                                                                                                  \
    {                                                                                                                   \
        Elf##x##_Sym *s = (Elf##x##_Sym *)(((unsigned char *)symtab->symbol) + (sizeof(elf_symbol_t) * symtab->count)); \
